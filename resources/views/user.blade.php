        ...
        <div id="middleArea">
            ...
            <section id="dashboard">
                ...
                <div class="dashboardImg">
                    <img src="{{ asset('storage/user_primary_img/'.$user->getPrimaryImg())}}" alt="profileImg">
                </div>
                <div class="dashboardName">
                    <div class="dashboardGreeting">{{$user->getUserName()}}</div>
                    <div class="dashboardComment">What are you up too?</div>
                </div>
                @forelse($liveList as $liveId => $live)
                <div>
                    <img src="{{ asset('storage/user_primary_img/'.$live->getUserPrimaryImg()) }}" alt="">
                    <p>{{ $live->getUserName }}</p>
                    <p>{{ $live->getLiveName() }}</p>
                    <p>{{ $live->getLivePlace() }}</p>
                    <p>{{ $live->getLiveStartTime() }}</p>
                    <p>{{ $live->getLiveEndTime() }}</p>
                </div>
                @empty
                @endforelse
                ...
                <div class="follow">
                    @auth
                        @if(in_array($user->getId(), $artistIdArray))
                            @if(Auth::user()->id == $user->getId())
                            @else
                            <follow-component :artist_id="{{$user->getId()}}" :is_follow="true"></follow-component>
                            @endif
                        @else
                            @if(Auth::user()->id == $user->getId())
                            @else
                            <follow-component :artist_id="{{$user->getId()}}" :is_follow="false"></follow-component>
                            @endif
                        @endif
                    @endauth
                </div>
